...
Works with both installed package and development setup.
"""
import sys
import os
    ...
    try:
        ...
        for hotel in resort.hotels:
            ...
            if success:
                ...
                print(
                    f"  Temp: {health_before['temperature']}°C → {health_after['temperature']}°C"
                )
                print(
                    f"  Current: {health_before['current']:.0f}mA → {health_after['current']:.0f}mA"
                )
                print(
                    f"  Position: {health_before['position']:.1f}° → {health_after['position']:.1f}°"
                )
